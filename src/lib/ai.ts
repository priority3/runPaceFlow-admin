...
/**
 * 活动库连接（activities / activity_insights 所在 shared.db）。
 *
 * Reason: shared.db 是活动数据唯一真相源；Turso 只由 mirror scheduler 做单向
 * 镜像/备份。生成与读取必须使用同一个 shared.db，否则主站看不到新缓存。
 */
async function getInsightsDb() {
